...
We provide two main kinds of segmentation -- LQ Seg & MM Seg.
(You can choose between them by switching the optionmenu below the menubar.)\n
And for segmentation, you can work with file seg & sentence seg.\n
...
def text_to_result(list_of_sentences):                           # seg file to final results.
    ...
    for i in range(len(total_text)):
        if total_text[i] in punc and total_text[i] not in '|《》（）()':
...
def segFile():                                                      # main entry 1
    ...
    if ask0 == True:
        ...
        while True:
            line_choose = simpledialog.askstring('Choose a sentence', 'Choose a sentence(0 to skip)',initialvalue='Enter number here')

            if line_choose == '0':
                break
            elif line_choose == None:
                break
            else:
                pass
    ...
    ask=messagebox.askyesno('Seg Text','Do you want to segment the whole text?')
    if ask == True:
        result=text_to_result(list_of_sentences)
        segText.delete(1.0, END)
        outputText.delete(1.0,END)
        outputText.insert(INSERT,result)
        messagebox.showinfo('Seg Text', 'The text has been segmented successfully')
        ask_later=messagebox.askyesno('Seg Text','Do you want to output the results into a new text?')

        if ask_later == True:
            new_built=open('output.txt','w')
            new_built.write(text_to_result(list_of_sentences))
            new_built.close()
            messagebox.showinfo('Seg Text','The new file has been built successfully')
...
Label(master,text='HKJ\nChinese Word Segmentation',height=3,font=('Times',25),fg='red').grid(row=0,column=0,columnspan=3)
...
select_system=system.get()
Label(master,text='Version 2.1',height=1,relief=RIDGE).grid(row=1,column=2,sticky=E,padx=10)
